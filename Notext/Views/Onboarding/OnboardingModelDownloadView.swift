...
struct OnboardingModelDownloadView: View {
    ...
    private let turboModel: LocalModel? = PredefinedModels.models.first { $0.name == "ggml-large-v3-turbo-q5_0" } as? LocalModel
                ...
                GeometryReader { geometry in
                    ...
                    VStack(spacing: 40) {
                        ...
                        VStack(spacing: 16) {
                            ...
                            SkipButton(text: isModelSet ? "Continue" : "Skip for now") {
                                withAnimation {
                                    showTutorial = true
                                }
                            }
    ...
    private func checkModelStatus() {
        guard let turboModel = turboModel else {
            isModelSet = true
            return
        }
    ...
    private func handleAction() {
        if isModelSet {
        ...
        } else if let turboModel = turboModel {
            if whisperModelManager.availableModels.contains(where: { $0.name == turboModel.name }) {
            ...
            // Model not found, just continue to tutorial
            withAnimation {
                showTutorial = true
            }
        }
