...
class HistoryWindowController: NSObject, NSWindowDelegate {
    ...
    private let windowIdentifier = NSUserInterfaceItemIdentifier("com.evecorp.notext.historyWindow")
    private let windowAutosaveName = NSWindow.FrameAutosaveName("NotextHistoryWindowFrame")

    private override init() {
        super.init()
    }

    func showHistoryWindow(modelContainer: ModelContainer, engine: NotextEngine) {
    ...
    private func createHistoryWindow(modelContainer: ModelContainer, engine: NotextEngine) -> NSWindow {
        ...
        window.title = "Notext — Transcription History"
